...
class TensorUtilsTestCase(unittest.TestCase):
  ...
  def test_create_tensor_from_df(self):
    ...
    self.assertFalse('B' in encoder_dict, "'B' should not be one-hot encoded.")
    self.assertTrue('A' in encoder_dict, "'A' should be one-hot encoded.")
    self.assertEqual(
        1, len(encoder_dict),
        "There should be only one variable to be one-hot encoded")
    self.assertTrue(
  ...
  def test_create_tensor_from_df_empty(self):
    ...
    self.assertFalse('C' in encoder_dict, "'C' should be one-hot encoded.")
    self.assertTrue(
        tf.reduce_all(tf.equal(tensor_true, tensor)),
        f"The created Tensor is not the same. Expect {tensor_true}, but get {tensor}."
    )
  ...
  def test_create_sparse_tensor(self):
    ...
    sparse_tensor = TensorUtils.csr_to_sparse_tensor(csr_matrix)
